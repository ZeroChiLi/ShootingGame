...
Shader "AO/ScreenSpaceAOEffect"
{
	...
	v2f vert_ao (appdata v)
	{
		...
		o.viewRay = viewRay.xyz / viewRay.w;		// 视线朝向
		return o;
	}
	...
	fixed4 frag_ao (v2f i) : SV_Target
	{
		...
		DecodeDepthNormal(cdn, linear01Depth, viewNormal);		// 计算深度 和法线
		...
		for(int i = 0; i < sampleCount; i++)
		{
			...
			float range = abs(randomDepth - linear01Depth) * _ProjectionParams.z < _SampleKeneralRadius ? 1.0 : 0.0;	// 如果深度相差大于半径，就不需要环境光遮蔽了
			float ao = randomDepth + _DepthBiasValue < linear01Depth  ? 1.0 : 0.0;		// 防止在平面上产生自阴影
			oc += ao * range;
		}
		oc /= sampleCount;	// 值越大遮蔽越多
